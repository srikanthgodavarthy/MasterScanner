...
def _stoch(df: pd.DataFrame, period: int, smooth: int) -> tuple[pd.Series, pd.Series]:
    ...
    study. [2026-07-29] fixed to smooth %K first so k_last/k_prev (used in
    Pillar 2 — Momentum Decay, which directly feeds the live Exit Strength
    Score / HOLD-REDUCE-EXIT recommendation) line up with what a
    TradingView chart with default (14, 3, 3) settings shows."""
    low_min = df["low"].rolling(period).min()
    high_max = df["high"].rolling(period).max()
    rng = (high_max - low_min).replace(0, np.nan)
    raw_k = (100 * (df["close"] - low_min) / rng).fillna(50)
