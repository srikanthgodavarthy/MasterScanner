...
def build_sector_benchmark_frames(close_by_symbol: dict, max_peers: int = 15,
    ...
    close_by_symbol : {symbol: pd.Series of close prices}, already fetched
                       ...
                       `all_data` dict, close column only). These Series
                       are typically multi-YEAR full history (needed
                       elsewhere for EMA200/trend-age); rebasing that
    ...
    lookback_bars   : each peer's close series is truncated to its LAST
                       ...
                       (5/21/63/126 bars, i.e. ~6 months + the momentum
                       function's extra 10-bar offset) — 210 bars is a
                       safe buffer above that. This bounds memory to a
                       small, fixed amount PER PEER regardless of how
                       many years of history close_by_symbol carries,
                       instead of the previous full-history rebase that
                       held up to ~300 multi-year Series in memory at
                       once (20 sectors x 15-peer cap).

    Returns : {sector_name: pd.DataFrame} — one column per peer symbol,
              rebased to 100 at that peer's first available date within
              the trimmed window. Sectors with fewer than 2 usable peers
              are omitted entirely (with only 1 peer, "leave one out"
              would leave nothing to average against) — callers must
              fall back to sector_series=None (the existing
              "unavailable" path) for those symbols.
    """
    frames: dict = {}
    for sector, syms in SECTOR_MAP.items():
        ...
        for p in syms:
            ...
            if s is None or len(s) == 0:
                continue
            if lookback_bars and len(s) > lookback_bars:
                s = s.iloc[-lookback_bars:]
            s = s.dropna()
            if s.empty or s.iloc[0] <= 0:
                continue
