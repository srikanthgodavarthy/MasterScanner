...
def lifecycle_from_scanner_row(
    ...
    return LifecycleRecord(
        ...
        # quality. `category` above already comes from `Recommendation`
        # (CV1 + Promotion Engine) — read CV1_* here too, or this record
        ...
        rs_composite  = _safe_float(row.get("RScomp",      row.get("rs_composite",  0.0))),
        adx           = _safe_float(row.get("ADX",          row.get("adx",           0.0))),
        bars_band     = str(row.get("BarsBand",   row.get("bars_band",   ""))),
        bars_since    = _safe_int(row.get("BarsSince",  row.get("bars_since",  0))),
        move_since    = _safe_float(row.get("MoveSince", row.get("move_since", 0.0))),
    )
