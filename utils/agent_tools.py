...
so lookups go through small `_first_col` / `_first_val` helpers that try
several candidate names rather than assuming one fixed schema.
"""
...
def get_live_scan_snapshot(symbol: str) -> dict:
    ...
    out = {
        ...
        "leadership":       _first_val(row, ["CV1_Leadership", "Legacy_Leadership", "DE_Leadership"]),
        "conviction":       _first_val(row, ["CV1_Conviction", "Legacy_Conviction", "DE_Conviction"]),
        "entry_quality":    _first_val(row, ["CV1_EntryQuality", "Legacy_EntryQuality", "Entry Quality", "DE_EntryQuality"]),
...
def list_live_signals(signal_class: str | None = None, top_n: int = 15) -> dict:
    ...
    lead_col   = _first_col(df, ["CV1_Leadership", "Legacy_Leadership", "DE_Leadership"])
    out_df = df.copy()
    ...
    cols = [c for c in [stock_col, class_col, lead_col,
                         _first_col(df, ["CV1_Conviction", "Legacy_Conviction", "DE_Conviction"]),
                         _first_col(df, ["CV1_EntryQuality", "Legacy_EntryQuality", "Entry Quality", "DE_EntryQuality"]),
