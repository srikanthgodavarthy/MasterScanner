...
def leadership_prescreen(df: pd.DataFrame, nifty: pd.Series, min_bars: int = 210) -> bool:
    ...
    v4's staged-elimination goal (Leadership as the primary gate, checked
    first) only reduces scan time if the elimination happens before the
    expensive part of the pipeline — build_indicators() computes EMA20/50/
    200, RSI, ATR, CCI, ADX (Wilder-smoothed, not vectorisable), Bollinger/
    Keltner bands, pivots, and fib levels for EVERY symbol regardless of
    whether Leadership will later reject it. Gating inside CV1 (which only
    ever sees the output of build_indicators) is too late to save that cost.
    ...
    scan with no diagnostic, so the bar is set well below v4's real
    Watch floor (60) — reject only symbols that are clearly weak on BOTH
    relative strength and trend direction, not borderline ones. Validate
    the false-negative rate against a full (non-prescreened) run before
    trusting this in a live scan you're relying on for the day's trades.
