...
with st.sidebar:
    ...
    st.divider()
    run_btn = st.button("▶ Run A/B Comparison", type="primary", use_container_width=True)
...
with col_l:
    with st.expander(f"Legacy trades ({leg['trade_count']})", expanded=False):
        st.dataframe(_safe_df(leg["trades"], _DISPLAY_COLS), use_container_width=True)

with col_r:
    with st.expander(f"Dispatch trades ({dis['trade_count']})", expanded=False):
        st.dataframe(_safe_df(dis["trades"], _DISPLAY_COLS), use_container_width=True)
